...
1. [2020/01/13-19](#20200113-19)
	1. [Eyeriss v2: A Flexible Accelerator for Emerging Deep Neural Networks on Mobile Devices](#eyeriss-v2-a-flexible-accelerator-for-emerging-deep-neural-networks-on-mobile-devices)
		1. [Hierarchical Architecture of Eyeriss](#hierarchical-architecture-of-eyeriss)
	1. [Chisel Syntax](#chisel-syntax)
		1. [MAC module Delay](#mac-module-delay)2020/01		
		1. [MEM Read Test](#mem-read-test)
		1. [Read After Read From Another Mem](#read-after-read-from-another-mem)
		1. [Read Part of `UInt`](#read-part-of-uint)
		1. [Combine Two Integers as Bits](#combine-two-integers-as-bits)
...
1. [2019/12/09-15](#20191209-15)
	1. [Git Command](#git-command)
		...
		1. [Other Git Commands](#other-git-commands)
...
This week I read [another paper of Eyeriss v2](#eyeriss-v2-a-flexible-accelerator-for-emerging-deep-neural-networks-on-mobile-devices) and found the new PE architecture it uses. So I changed the design to fit this new one. And because of the complexity of CSC coding format it use, I spent almost two days to work out the read control logic of CSC hence no time for the software analysis system. And today I thought out a more hardware-friendly CSC format, and I will work on it. Currently, this PE project has passed two basic read and write tests.
...
The PEs and GLBs are grouped into clusters in order to support a flexible on-chip network (NoC) that connects the GLBs to the PEs at low cost.
...
+ For situations where the data reuse cannot fully utilize the entire PE array with broadcast, different multicast modes, specifically grouped-multicast and interleaved multicast, can be adopted according to the desired multicast patterns.
...
![Hierarchical mesh network for psums. This only shows the a 2×2 portion of the entire 8×2 cluster array](https://images-cdn.shimo.im/HdYFi0MzVIgkyYm0/image.png)
...
### Read After Read From Another Mem
...
`git pull origin master`: download the newest project from remote and merge it to current branch.

`git stash`: ignore the current changes and then can switch to other branch.
